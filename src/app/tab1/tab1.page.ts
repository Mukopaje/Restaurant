...
export class Tab1Page implements OnInit, OnEnter, OnDestroy {
    ...
    rootRef
      ...
      .then((snapshot) => {
        ...
        snapshot.forEach((item) => {
          ...
          for (const i of pickup_place_times) {
            const pickup_place_time = pickup_place_times[i];
            console.log(pickup_place_time);
            if (pickup_place_time == null) {
              continue;
            }

            const location = pickup_place_time.location;
            const beginTime = pickup_place_time.begin_time;
            const endTime = pickup_place_time.end_time;
  ...
  filterRestaurant(
    ...
    if (
      userLocation.toLowerCase().localeCompare(location.toLowerCase()) !== 0
    ) {
      selected = false;
    }
