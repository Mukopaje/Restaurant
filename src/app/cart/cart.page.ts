          ...
          .then(() => {
            this.storage.get('location').then(async (locVal) => {
              ...
              this.pickup_place_times.forEach((pickup) => {
                ...
                if (
                  locVal &&
                  locVal.toLowerCase() === pickup.location.toLowerCase()
                ) {
                  preselect = true;
                  input.checked = true;
                }
  ...
  populateItems() {
    this.storage.get('category').then((userCat) => {
      ...
      this.storage.get('cart').then((cart_array) => {
        this.items = cart_array.filter((item) => item.meal_type === userCat);
      });
  ...
  confirmOrder(alertData, typeVal, restaurant_id) {
    ...
    firebase.default
      ...
      .then((snapshot) => {
        ...
        const rootRef = firebase.default.database().ref();
        rootRef
          .child('restaurants/' + restaurant_id)
          .once('value')
          .then((snapshot2) => {
            const restaurant_name = snapshot2.val().name;
            const newOrderKey = rootRef.child('orders/').push().key;
            rootRef
              .child('orders/' + newOrderKey)
              .set({
